...
args.add_argument('--spline', default='linear', type=str, help='interpolation')
args.add_argument('--depth', default=2, type=int, help='RDE depth')
args.add_argument('--wnd_len', default=2, type=int, help='RDE window_size')
...
path = '../runs'
# model_save_name = join(join(path, save_name), 'model_save/')
# args.log_dir = join(path, save_name, args.dataset)
...
if args.model_type=='rde':
    ...
    model, vector_field_f, vector_field_f_prime, vector_field_g = make_model(args)
elif args.model_type=='type2gg':
    model, vector_field_f, vector_field_f_prime, vector_field_g = make_model(args)
elif args.model_type=='type1_zprime':
    model, vector_field_f, vector_field_g, vector_field_g_prime = make_model(args)
...
if args.model_type=='type1_temporal':
    ...
    vector_field_g = None
elif args.model_type in ('type1_spatial', 'fft_spatial'):
    vector_field_f = None
    vector_field_g = vector_field_g.to(args.device)
#TODO: only get the regulaization one
else:
    vector_field_f = vector_field_f.to(args.device)
    vector_field_g = vector_field_g.to(args.device)
...
if args.model_type in ('fft','fft_spatial'):
    trainer = Trainer_fft(model, vector_field_f, vector_field_g, loss, optimizer, train_loader, val_loader, test_loader, scaler,
