...
class FinalTanh_f_rde(nn.Module):
    def __init__(self, input_channels, hidden_channels, hidden_hidden_channels, num_hidden_layers):
        super(FinalTanh_f_rde, self).__init__()
...
class VectorField_g_rde(torch.nn.Module):
    def __init__(self, input_channels, hidden_channels, hidden_hidden_channels, num_hidden_layers, num_nodes, cheb_k, embed_dim,
                    g_type):
        super(VectorField_g_rde, self).__init__()
...
class FinalTanh_f_fft(nn.Module):
    def __init__(self, input_channels, hidden_channels, hidden_hidden_channels, num_hidden_layers):
        super(FinalTanh_f_fft, self).__init__()
        ...
        self.linears = nn.ModuleList(torch.nn.Linear(hidden_hidden_channels, hidden_hidden_channels)
                                           for _ in range(num_hidden_layers - 1))
        self.linear_out = nn.Linear(hidden_hidden_channels, input_channels * hidden_channels) #32,32*4  -> # 32,32,4 
        # self.linear_out_dft = torch.nn.Linear(hidden_hidden_channels, input_channels * hidden_channels) #32,32*4  -> # 32,32,4 
    ...
    def forward(self, t, z):
        # dft_X = z[...,self.hidden_channels:]
        # z = z[...,:self.hidden_channels]
        ...
        # dft_X = self.linear_out_dft(dft_X).view(*dft_X.shape[:-1], self.hidden_channels, self.input_channels)
        # dft_X = dft_X.tanh()
        # z = torch.cat([z, torch.zeros_like(dft_X)],dim=-2)

        return z

class FinalTanh_f_fft2(nn.Module):
    def __init__(self, input_channels, hidden_channels, hidden_hidden_channels, num_hidden_layers):
        super(FinalTanh_f_fft2, self).__init__()
...
class VectorField_g_fft(torch.nn.Module):
    def __init__(self, input_channels, hidden_channels, hidden_hidden_channels, num_hidden_layers, num_nodes, cheb_k, embed_dim,
                    g_type):
        super(VectorField_g_fft, self).__init__()
        ...
        #FIXME:
        self.split_opt = True
        self.only_spatial = False

        if self.split_opt == True:
            self.hidden_channels = hidden_channels
            self.linear_in = torch.nn.Linear(hidden_channels, hidden_hidden_channels)
        elif self.split_opt == False:
            self.hidden_channels = hidden_channels
            self.hidden_channels_twice = hidden_channels * 2
            self.linear_in = torch.nn.Linear(self.hidden_channels_twice, hidden_hidden_channels)
        
        # self.linears = torch.nn.ModuleList(torch.nn.Linear(hidden_hidden_channels, hidden_hidden_channels)
        #                                    for _ in range(num_hidden_layers - 1))


        if self.only_spatial == True:
            self.linear_out = torch.nn.Linear(hidden_hidden_channels, input_channels * hidden_channels) #32,32*4  -> # 32,32,4 
            self.linear_out_dft = torch.nn.Linear(hidden_channels, input_channels * hidden_channels) #32,32*4  -> # 32,32,4 
        elif self.only_spatial == False:
            if self.split_opt == True:
                self.linear_out = torch.nn.Linear(hidden_hidden_channels, self.hidden_channels * self.hidden_channels) #32,32*4  -> # 32,32,4 
            else:
                self.linear_out = torch.nn.Linear(hidden_hidden_channels, self.hidden_channels * self.hidden_channels_twice) #32,32*4  -> # 32,32,4 
            self.linear_out_dft = torch.nn.Linear(hidden_channels, hidden_channels * hidden_channels) #32,32*4  -> # 32,32,4 
    ...
    def forward(self, t, z):
        ...
        if self.split_opt == True:
            ...
            if self.only_spatial == True:
                ...
                z = self.linear_out(z).view(*z.shape[:-1], self.hidden_channels_twice, self.hidden_channels)
        ...
        if self.split_opt == True:
            if self.only_spatial == True:
                ...
                dft_X = self.linear_out_dft(dft_X).view(*dft_X.shape[:-1], self.hidden_channels, self.hidden_channels)
            dft_X = dft_X.tanh()
            z = torch.cat([z, torch.zeros_like(dft_X)],dim=-2)
 
        # z = torch.cat([z, dft_X],dim=-2)
...
class VectorField_g_fft_spatial(torch.nn.Module):
    def __init__(self, input_channels, hidden_channels, hidden_hidden_channels, num_hidden_layers, num_nodes, cheb_k, embed_dim,
                    g_type):
        super(VectorField_g_fft_spatial, self).__init__()
